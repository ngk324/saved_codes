...
def count_unique_nodes(directory):
    ...
    edges = get_edges(directory)
    for i in range(len(edges)):
        source_node = egdes[i][0]
        target_node = egdes[i][1]
...
def count_edges_in_subgraph(directory, subgraph_nodes):
    ...
    edge_count = 0
    edges = set()
    edges_list = get_edges(directory)
    for i in range(len(edges_list)):
        source_node = edges_list[i][0]
        target_node = edges_list[i][1]
...
def find_largest_1hop_subtree_by_edges(directory):
    ...
    edges_list = get_edges(directory)
    for i in range(len(edges_list)):
        source_node = egdes_list[i][0]
        target_node = egdes_list[i][1]
        graph_nodes.add(source_node)
        graph_nodes.add(target_node)
...
def get_neighbors(directory, subgraph_nodes, node=0):
    ...
    edges = count_edges_in_subgraph(directory, subgraph_nodes)
    nbhs = set()
